...
app.get("/returnDetailsOfAPost", async (req, res) => {
...
app.post("/insertCommentByUserAtAPost", async (req, res) => {
  console.log(req.body);
...
app.get("/fetchAllCommunities", verifyToken, (req, res) => {
  client.query("select * from communities", (errors, result) => {
    if (errors) {
      res.status(500).send("Error fetching all communities from database");
    } else {
...
app.post(
  ...
  async (req, res) => {
    ...
    let creatorId = await client.query("select * from users where email=$1", [
      creatorEmail,
    ]);
